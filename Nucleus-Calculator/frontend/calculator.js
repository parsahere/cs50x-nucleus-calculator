...
let calcBody_blur = function (add_or_remove) {
    if (add_or_remove == "add") {
        calcBody.classList.add("but-active")
        output_element.classList.add("output-inactive")
    } else if (add_or_remove == "remove") {
        calcBody.classList.remove("but-active")
        output_element.classList.remove("output-inactive")
    } else {
        console.error(
            "The argument passed to calcBody_blur function is not defined!"
        )
    }
}
...
input_element.addEventListener("click", (event) => {
    ...
    calculator_buttons.forEach((button) => {
        if (button.name == target_btn.className) {
            calculator_function(button)
        }
    })
})
...
function calculator_function(button) {
    ...
    if (ans_check_click == 1) {
        lastAns(last_answer)
    } else if (ans_check_click == 0) {
...
// THE ENTIRE CODEBASE IS ORIGINALLY WRITTEN BY THE DEVELOPER OF NUCLEUS CALCULATOR APP, PARSA SABET.